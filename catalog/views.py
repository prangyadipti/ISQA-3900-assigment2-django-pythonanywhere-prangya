...
# UpdateView for updating a Book
...
def book_delete(request, pk):
    book = get_object_or_404(Book, pk=pk)  # Changed to Book
    try:
        book.delete()
        messages.success(request, f'{book.title} has been deleted')
    except:
        messages.error(request, f'{book.title} cannot be deleted. This book may have related records.')
